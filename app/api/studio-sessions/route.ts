...
type PhotoPreflight={score:number;overall_score:number;base_score:number;raw_score?:number;applied_cap?:{id:string;label:string;cap:number}|null;score_trace?:string[];status:"APPROVED"|"REVIEW"|"REUPLOAD"|"REJECT";label:string;confidence:number;technical_quality:number;file_suitability?:number;file_status?:"OK"|"LOW"|"TOO_SMALL"|"UNUSABLE";file_reason?:string;body_usability?:number;face_visibility?:number;editability?:number;body_extent?:string;hands?:string;professionalism:number;composition:number;background_quality:number;face_quality:number;designer_usability:number;pose_appropriateness:number;selfie_probability:number;issues:string[];strengths:string[];recommendation:string;decision_reason:string;requirements:PhotoRequirement[];penalties:PhotoPenalty[];metrics:RatingMetric[]};
