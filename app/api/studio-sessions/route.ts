type StudioSession={session:string;agentId:string;agentName:string;agentPhoto?:string;rating?:number;date:string;time:string;createdAt:string;status:"confirmed"};
...
export async function POST(request:Request){
 try{
  ...
  const record:StudioSession={session:body.session,agentId:body.agentId,agentName:body.agentName,agentPhoto:body.agentPhoto,rating:body.rating,date:body.date,time:body.time,createdAt:new Date().toISOString(),status:"confirmed"};
