...
export async function POST(request:Request){
 ...
 try{
  const body=await request.json() as {reminders?:{reminderId?:string;agentId?:string;agentName?:string;intendedRecipientEmail?:string;uploadUrl?:string;optOutUrl?:string}[]},reminders=body.reminders;
  ...
  const origin=(process.env.STUDIO_PUBLIC_URL?.trim()||new URL(request.url).origin).replace(/\/$/,""),messages=reminders.map(item=>{const agentName=String(item.agentName??"").slice(0,160);return{id:String(item.reminderId??""),agentId:String(item.agentId??"").slice(0,120),agentName,intendedRecipientEmail:String(item.intendedRecipientEmail??"").slice(0,254),actualRecipientEmail:config.recipient,subject:`[TEST] Reminder: Upload Your Profile Lab AI Profile Photo — ${agentName}`,uploadUrl:new URL(String(item.uploadUrl??"/"),`${origin}/`).href,optOutUrl:new URL(String(item.optOutUrl??"/"),`${origin}/`).href}});
