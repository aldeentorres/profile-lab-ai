...
function mimeMessage({from,to,subject,text,html}){const boundary=`studio-plus-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`,domain=address(from).split("@")[1]||"localhost";return [`Date: ${new Date().toUTCString()}`,`Message-ID: <${crypto.randomUUID()}@${domain}>`,`From: ${safeHeader(from)}`,`To: ${safeHeader(to)}`,`Subject: ${headerWord(subject)}`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${boundary}"`,"",`--${boundary}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",base64Lines(text),`--${boundary}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",base64Lines(html),`--${boundary}--`,""].join("\r\n")}
...
export async function sendSmtpMessages(input,messages){
 ...
 try{
  ...
  for(const message of messages){const to=address(message.to);await command(socket,replies,`MAIL FROM:<${address(config.from)}>`,[250]);await command(socket,replies,`RCPT TO:<${to}>`,[250,251]);await command(socket,replies,"DATA",[354]);const mime=mimeMessage({from:config.from,to,subject:message.subject,text:message.text,html:message.html}),stuffed=mime.split("\r\n").map(line=>line.startsWith(".")?`.${line}`:line).join("\r\n");socket.write(`${stuffed}\r\n.\r\n`);await expect(replies,[250]);deliveredIds.push(message.id)}
