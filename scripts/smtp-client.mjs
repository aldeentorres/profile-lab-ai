...
function mimeMessage({from,to,subject,text,html,inlineImages=[]}){const unique=`${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`,alternative=`profile-lab-ai-alt-${unique}`,related=`profile-lab-ai-rel-${unique}`,domain=address(from).split("@")[1]||"localhost";
...
async function connect(config){
 ...
 const plain=typeof config.socketFactory==="function"?config.socketFactory():net.connect({host:config.host,port:config.port});await waitFor(plain,"connect");let replies=new Replies(plain);await expect(replies,[220]);await command(plain,replies,"EHLO profile-lab-ai.local",[250]);if(config.security==="none")return{socket:plain,replies,greeted:true};await command(plain,replies,"STARTTLS",[220]);replies.close();const socket=tls.connect({socket:plain,servername:config.host,rejectUnauthorized:true});await waitFor(socket,"secureConnect");replies=new Replies(socket);return{socket,replies}
}
...
export async function sendSmtpMessages(input,messages){
 ...
 try{
  ...
  if(!connection.greeted)await command(socket,replies,"EHLO profile-lab-ai.local",[250]);
