...
test("the local SMTP adapter sends every personalised reminder to one test inbox",async()=>{
 ...
 assert.deepEqual(await sendSmtpMessages({host:"127.0.0.1",port:2525,security:"none",auth:"none",from:"Profile Lab AI <studio@example.test>",socketFactory},messages),["one","two"]);assert.deepEqual(recipients,[`RCPT TO:<${to}>`,`RCPT TO:<${to}>`]);assert.deepEqual(subjects,["Reminder one","Reminder two"])
});
